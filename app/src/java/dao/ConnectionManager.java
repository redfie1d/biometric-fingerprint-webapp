...
//        Logger.getLogger(ConnectionManager.class.getName()).log(Level.INFO, message);
    ...
    public static void sendImage(String imgOutgoing) {
        try {
            System.out.println("Initiating link to host machine...");
            URL url = new URL("http://" + dbURL + "/SabaiBiometrics/ImageReceiver");
            ...
            os.write(imgOutgoing.getBytes());
            os.flush();
            os.close();
            // For POST only - END
            System.out.println("Finished sending the image to host...");
